...
def get_laplace_matrix(x):
    degree_matrix = np.diag(np.sum(x, axis=0))
    return degree_matrix - x

def add_gaussian_noise(matrix, std_noise=0):
    if sparse.issparse(matrix):
        matrix = matrix.toarray()
    else:
        matrix = np.asarray(matrix)
    mean_noise = np.mean(matrix[matrix != 0])
...
def torch_soft(x, th):
    y = torch.clamp(torch.abs(x) - th, min=0.)
    return torch.sign(x) * y

# -------------------------
#  Adam implementation with Pytorch
def adam(w, dw, config=None):
    ...
    config.setdefault('m', torch.zeros_like(w, device=w.device))
    config.setdefault('v', torch.zeros_like(w, device=w.device))
...
# -------------------------
# Contrastive gradients computation
# -------------------------
def contrastive_grad(a, neig_indicator, tau=1.0):
    N, _ = a.shape
    da = torch.zeros_like(a)

    n_i = neig_indicator.sum(dim=1)  # shape (N,)
    exp_all = torch.exp(a / tau)
    exp_neg = exp_all * (~neig_indicator.bool())
    denom = exp_neg.sum(dim=1, keepdim=True) + 1e-12

    da[neig_indicator.bool()] = -1.0 / (2.0 * N * tau)
    coeff = (n_i / (2.0 * N * tau)).unsqueeze(1)
    da += coeff * (exp_neg / denom)

    return da

def compute_da(a, neig_indicator, beta, eta, fg,epoch):
    tau = 1.0
    da = 2 * beta * (-fg.T @ fg + fg.T @ fg @ a) + 2*a
    neig_indicator = (neig_indicator != 0).int()
    da_contrastive = contrastive_grad(a=a, neig_indicator=neig_indicator, tau=tau)
    da += beta * da_contrastive
    return da
...
def to_tensor(v,device):
    if isinstance(v, (np.ndarray, np.generic)):
        ...
        return v.to(device=device, dtype=torch.float64)
    else:
        return v
def to_tensor_32(v,device):
    if isinstance(v, (np.ndarray, np.generic)):
...
def priori_cluster(
    ...
    for res in sorted(np.arange(0.1, 2.5, 0.01), reverse=True):
        sc.tl.leiden(adata, random_state=0, resolution=res, flavor="igraph", n_iterations=2, directed=False,neighbors_key='representation')
...
# for reproducntion of SEDR without R pacakage
def mclust_py(adata, n_clusters, use_rep='SEDR', key_added='SEDR', random_seed=0):
    seed_setting(42)
    gmm = GaussianMixture(
    ...
    labels = gmm.fit_predict(adata.obsm[use_rep])
